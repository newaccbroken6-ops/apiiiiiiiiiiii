...
async def EncRypTMajoRLoGin(open_id, access_token):
    ...
    major_login.client_version = "1.118.9"
                ...
                if data.hex().startswith("120000"):
                    ...
                    if response:
                        ...
                        if inPuTMsG in ("hi" , "hello" , "fen" , "salam"):
                            uid = response.Data.uid
                            chat_id = response.Data.Chat_ID
                            message = 'linux ya baba hada API a w 9'
                            P = await SEndMsG(response.Data.chat_type , message , uid , chat_id , key , iv)
                            await SEndPacKeT(whisper_writer , online_writer , 'ChaT' , P)
                        response = None
...
async def perform_emote(team_code: str, uids: list, emote_id: int):
    ...
    try:
        ...
        await asyncio.sleep(0.3)  # Reduced from 1 second
        ...
        # Exit the squad after performing emotes
        E = await ExiT(None, key, iv)
        await SEndPacKeT(None, online_writer, 'OnLine', E)
        
        return {"status": "success", "message": "Emote performed successfully and bot exited!"}
    except Exception as e:
...
async def MaiiiinE():
    ...
    Uid , Pw = '4197713441' , '6478B1F1D3BC4CB0AC89079BA3297E29B3285BEEED3A87F7191B8AE277D6DD55'
